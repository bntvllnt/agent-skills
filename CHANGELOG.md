...
- feat(prepare-github-repository): enforce commit-email privacy so a personal email can never be leaked through commit metadata — force the repo commit identity to the GitHub no-reply address (`<id>+<login>@users.noreply.github.com`), add a native server-side **ruleset** `commit_author_email_pattern` + `committer_email_pattern` rule that makes GitHub reject any push with a non-no-reply commit, plus layered fallbacks: account-level "block pushes that expose my email", a CI email-guard workflow required as a status check, and an optional pre-push hook. Adds `references/email-privacy.md`, `templates/rules/commit-privacy.md`, and `templates/ci/email-guard.yml`; new desired-state items #14 (commit identity) and #15 (email-leak guard).
